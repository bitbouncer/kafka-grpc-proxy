...
namespace bb {
  struct producer_user_data {
    producer_user_data(void *key, size_t keysz, void *val, size_t valsz, uint32_t hash)
        : key_ptr(key), key_sz(keysz), val_ptr(val), val_sz(valsz), partition_hash(hash) {
    }
  ...
  kafka_producer::kafka_producer(std::shared_ptr<kspp::cluster_config> cconfig, std::string topic)
      : topic_(topic), msg_cnt_(0), msg_bytes_(0), closed_(false), nr_of_partitions_(0) {
    LOG_IF(FATAL,
           cconfig->get_cluster_metadata()->wait_for_topic_leaders(topic, cconfig->get_cluster_state_timeout()) ==
           false) << "failed to wait for topic leaders, topic:" << topic;
  ...
  kafka_producer::produce(uint32_t partition_hash, memory_management_mode mode, void *key, size_t keysz, void *value,
                          size_t valuesz, int64_t timestamp) {
    ...
    if (ec == RdKafka::ERR__QUEUE_FULL) {
      DLOG(INFO) << "kafka_producer, topic:" << topic_ << ", queue full - retrying, msg_count (" << msg_bytes_ << ")";
      delete user_data;
      return ec;
